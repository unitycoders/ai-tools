		...
		if (!dead) {
			...
			if (!running) {
				if (growThreshold < timeLived) {
					...
					int distance = maxRunnerDistance
							- (int) (world.map[x][y] * maxRunnerDistance);
					for (int i = 0; i < numberOfRunners; i++) {
						// Vector2D runnerTarget = new
						// Vector2D(Math.toRadians(Math.random() * 360), 1,
						// true);
						Vector2D runnerTarget = Vector2D.getRandomPolar(
								2 * Math.PI, 1, 1, true);
						...
						if (runnerTarget.x < 0 || runnerTarget.x > world.width
								|| runnerTarget.y < 0
								|| runnerTarget.y > world.height) {
							i--;
							continue;
						}
						...
						currentRunnerLocations
								.add(new Vector2D(location, true));
					}
					running = true;
				...
				for (int i = 0; i < targetLocations.size(); i++) {
					if (!runnerCompleted[i]) {
						...
						if (difference.mag() > 6) {
							...
							// difference.mag());
							currentRunnerLocations.get(i).add(
									Vector2D.subtract(targetLocations.get(i),
											location),
									3 / targetLocations.get(i).mag());
						} else {
							// System.out.println("Strawberry lived for" +
							// timeLived);
							world.addStrawberry(new Strawberry(targetLocations
									.get(i)));
	...
	public void draw(Graphics2D graphics2D) {
		...
		if (!dead) {
			for (int i = 0; i < currentRunnerLocations.size(); i++) {
				...
				// graphics2D.fillRect((int) targetLocations.get(i).x, (int)
				// targetLocations.get(i).y, 5, 5);
