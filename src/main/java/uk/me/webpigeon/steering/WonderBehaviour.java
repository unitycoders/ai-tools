...
public class WonderBehaviour implements SteeringBehaviour {
	...
	public Vector2D process() {
		...
		Vector2D pw = Vector2D.add(ourPosition,
				Vector2D.multiply(v, wanderDistance));
		...
		// set the new target
		Vector2D seekTarget = Vector2D
				.add(Vector2D.add(ourPosition,
						Vector2D.multiply(v, wanderDistance)), wanders);
	...
	public void debugDraw(Graphics2D g) {
		...
		g.drawOval((int) wanders.x, (int) wanders.y, (int) wanderRadius * 2,
				(int) wanderRadius * 2);
